...
async function loadDepartmentsForDropdown() {
    try {
        ...
        // Clear existing options
        [doctorDeptSelect, staffDeptSelect, doctorSearchDept, staffSearchDept].forEach(select => {
            if (select) {
                select.innerHTML = select.innerHTML.split('</option>')[0] + '</option>';
            }
        });
        ...
        departments.forEach(dept => {
            ...
            if (doctorSearchDept) doctorSearchDept.innerHTML += option;
            if (staffSearchDept) staffSearchDept.innerHTML += option;
        });
    } catch (error) {
        console.error('Error loading departments:', error);
    }
